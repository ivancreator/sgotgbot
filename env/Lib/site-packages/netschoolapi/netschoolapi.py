...
class NetSchoolAPI:
    ...
    async def login(self, cid: Union[str, int], sid: Union[str, int], pid: Union[str, int], cn: Union[str, int], sft: Union[str, int], scid: Union[str, int], user_name: str, password: str):
        ...
        if 'message' in login_meta:
            raise errors.NetSchoolAPIError(login_meta['message'])
        ...
        try:
            ...
            if http_status_error.response.status_code == httpx.codes.CONFLICT:
                ...
                if 'message' in response.json():
                    raise errors.AuthError(response.json()['message'])
                else:
        ...
        self._login_data = (cid, sid, pid, cn, sft, scid, user_name, password)
        ...
        self._student = self._students[diary_info['currentStudentId']]
    ...
    async def _address(self, school: Union[str, int]) -> Dict[str, int]:
        try:
            ...
            if http_status_error.response.status_code == 404:
                preresponse = await self._client.get(
                    'prepareloginform'
                )
                # cid = response.json()["countries"]["cid"]
                # sid = response.json()["countries"]["sid"]
                # pid = response.json()["provinces"]["pid"]
                # response = await self._client.get(
                #     'loginform?cid=2&sid=66&pid=-3&cn=35&sft=1&LASTNAME=sft'
                # )
                # schools_reference = []
                schools_reference = preresponse.json()['schools']
